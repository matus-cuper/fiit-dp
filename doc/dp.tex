...
%TODO for NN \subsubsection{Normalizácia číselných vektorov}
...
základe súrových dát, ktoré máme k~dispozícií môžu nastať dva rôzne scenáre. Ak
dataset bude obsahovať iba časovú známku a~spotrebu elektrickej energie daného
zákazníka, zhlukovanie bude možné iba na základe časového radu spotreby
a~výsledky budú evaluované pomocou vzdialeností medzi jednotlivými časovými
radmi vo vnútri zhlukov. Naopak, ak dataset bude obsahovať viaceré vysvetľujúce
premenné, potom je možné vytvoriť model, ktorý bude zhlukovať odberateľov na
základe týchto atribútov. Tak bude zabezpečená evaluácia pôvodného zhlukovacieho
modelu.

Vytvorené zhluky tvoria dve skupiny zhlukov, deliacim kritériom je hustota
vytvorených zhlukov. Primárne sa budeme zameriavať na analýzu časových radov,
ktoré spadajú do riedkych zhlukov. Cieľom je v~takýchto časových radoch
identifikovať intervaly s~neštandardným správaním odberateľa. Musíme pri tom
brať ohľad najmä na cyklus dní a~týždňov, no zároveň pristupovať k~zvykom
odberateľov jednotlivo a~zvážiť ich pri označení intervalu. Samotné
diskretizované časové rady môžeme považovať za anomálie, ale zameriavame sa
najmä na lokalizáciu problému a~minimalizáciu identifikovaného intervalu.
...
mechanizmus na určovanie intervalu, v ktorom sa vyskytuje anomália.
...
\begin{description}
  \item[$\bullet$ vizualizácia anomálií] je potrebná najmä pre pochopenie
	dôvodov, pre ktoré vznikli anomálie
  \item[$\bullet$ validácia výsledkov] prebieha na viacerých úrovniach procesu
\end{description}

\noindent
\paragraph{Nefunkcionálne požiadavky:}
\begin{description}
  \item[$\bullet$ univerzálnosť] riešenia je výsledkom detekcie anomálií bez
	učiteľa z~časových radov, ktoré sa spoliehajú na minimálny počet dostupných
	atribútov
	\item[$\bullet$ robustnosť] implementácie je kľúčová vzhľadom na rôznorodosť
	vstupných dát
  \item[$\bullet$ konfigurovateľnosť] aplikácie zabezpečujú viacere rozhrania
