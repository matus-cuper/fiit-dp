...
\begin{figure}[htbp]
  ...
  \caption[Príklad porovnávania časových radov pomocou dynamickej deformácií času.]{Príklad porovnávania časových radov pomocou dynamickej deformácií času~\cite{Malinowski2017}.}
...
\begin{figure}[htbp]
  ...
  \caption[Príklad porovnávania časových radov pomocou Euklidovej vzdialenosti.]{Príklad porovnávania časových radov pomocou Euklidovej vzdialenosti~\cite{Malinowski2017}.}
...
vzdialenosti sú popísané v~kapitole~\ref{c:distance-metrics}. Kritériami na
výber je presnosť a~rýchlosť výpočtu, prípadne schopnosť spracovať aj časové
...
\subsection{Ohodnotenie podozrivých zhlukov a~intervalov}
...
\subsection{Selekcia podozrivých inštancií}
...
\begin{equation}
  RLE = dlzka_{RLE}(casovy\_rad_i) - 1
...
\subsection{Vyhladenie časových radov}
...
\subsection{Analýza metódou S-H-ESD}
Cieľom je vylepšiť skóre vypočítané v~predchádzajúcich krokoch a~spresniť tak
interval, v~ktorom je výskyt anomálií výrazne väčší. Výstupom je pôvodný časový
rad s~novým atribútom, ktorý pre každé meranie obsahuje skóre opisujúce
podozrenie na anomálnosť. Atribút budeme nazývať skóre odberateľa, nakoľko je
...
% TODO add some equation, mozno nejaky logaritmus per window + jednotlive instancie
\begin{equation}
  \label{eq:client-score}
\end{equation}

Dosiahnuté výsledky je nutné vhodne vizualizovať a~vyhodnotiť. Príkladom môže
byť vizualizácia výsledkov na obrázku ~\ref{fig:shesd-example} TODO pridat obrazok.
% TODO add picture and description
Vzhľadom na fakt, že metóda je založená na učení bez učiteľa, vyhodnocovanie prebieha na základe
už spomenutých riešení, konkrétne FeaClip transformácie a~metóde S-H-ESD.

% \begin{figure}
%   \centering
%   \includegraphics[width=\textwidth]{shesd_example.png}
%   \caption{TODO add caption .}
%   \label{fig:shesd-example}
% \end{figure}
...
\begin{figure}
  ...
  \caption{Vizualizácia identifikovaných anomálnych odberateľov pomocou PCA.}
