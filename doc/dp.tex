...
% TODO make hidden Markov model paragraph
...
\begin{figure}[]
  ...
  \caption{Príklad porovnávania časových radov pomocou Euklidovskej vzdialenosti~\cite{Hooper2010}.}
...
\subsubsection{Kombinované učenie}
...
Existuje niekoľko techník, používaných pri klasifikátoroch s~kombinovaným učením.
...
a~samotrénovacie techniky s~kooperáciou (angl. co-training) techniky~\cite{Gonzalez2017}.
...
\textbf{prístup k učeniu}, ktorý môže byť jednoduchý alebo viacnásobný, ktorý je
aplikovateľný iba pri multi-klasifikátorových
...
V~prípade, že by boli k~dispozícií všetky parametre distribučnej sieti, bolo by
možné vypočítať aj presné množstvo technických strat. Medzi parametrami by
museli byť aj jednotlivé body pripojenia všetkých zákazníkov, topológia siete,
ale aj nameraný odpor medzi jednotlivými uzlami v~sieti či odber energie
zákazníkov. Táketo pokrytie parametrov je v~praxi ekonomicky extrémne náročné.
Aj keď inteligentné merače sa stávajú čoraz viac rozšírenými, ich prípadná
porucha vnáša do sieti nové premenné, ktoré nie je možné zmerať. Aj z~tohto
dôvodu nie je ľahké jednoznačne oddeliť technické a~netechnické straty v~sieti.
V~práci sa preto ďalej budeme zameriavať na anomálie vo všeobecnosti, nezávisle
na tom, či sú spôsobené poškodením meracích zariadení alebo neoprávnenou
manipuláciou zákazníka~\cite{Nikovski2013}.

%-------------------------------------------------------------------------------

Na detekciu anomálií sú používané nasledujúce metódy~\cite{Spiric2015}.
...
\subsubsection{Premiestňovanie zhlukov}
Algoritmus pozostáva z~nasledujúcich 3~krokov:
1. Začnime s~počiatočným zhlukovaním $C$, ktoré má predpísané konečný počte
zhlukov $k$
2. Pre každý časový rad vypočítajme maticu rozdieľnosti, výsledok si uložme
a~vypočítajme trajektóriu podobnosti
3. Vytvorme zhlukovanie $C'$, ktoré bude lepšie ako $C$ na základe používanej
metriky z~\ref{c:distance-metrics}, ak takéto zhlukovanie neexistuje,
nepokračujeme, inak prepíšeme pôvodné zhlukovanie a~skočíme na krok 1
Zhlukovanie $C'$ vzniká z~pôvodného zhlukovania $C$ presunom časových radov
medzi jednotlivými zhlukmi alebo ich výmenou. Algoritmus je aplikovatľný iba na
časové rady rovnakej dĺžky, keďže vzdialenosť na priereze časových radov nie je
definovaná~\cite{WarrenLiao2005}.
...
v~závislosti od tvaru časového radu~\cite{ }. % TODO pridaj Keogh et al. (2000).1 Keogh et al. (2001a)
...
Cieľom dolovania v~dátach je odhaliť skryté informácie z~pôvodných alebo
transformovaných časových radov. Najčastejšou úlohou je identifikácia vzorov
v~časových radoch, ale aj ich zhlukovanie, klasifikácia alebo
sumarizácia~\cite{Fu2011}.
