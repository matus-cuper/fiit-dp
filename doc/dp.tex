...
\usepackage[justification=centering]{caption}																		% enable caption centering
...
\hyphenation{veľ-kosť}
\hyphenation{veľ-kos-ťou}
\hyphenation{naj-čas-tej-šie}
\hyphenation{ap-proach}
\hyphenation{sku-toč-ným}
\hyphenation{jed-no-znač-ne}
\hyphenation{Fea-Clip}
\hyphenation{dtw-clust}
\hyphenation{dos-tup-ných}
\hyphenation{pos-led-né}
...
{Supervisor: \indent \indent \vphantom{} Marek Lóderer \par}
{April 2019 \par}
\bigskip
...
znižujú straty pri dodávke energie a~zároveň dokážu preverovať iba odberateľov,
...
také intervaly, ktoré svojou charakteristikou nezodpovedajú správaniu ostatných
...
prekročení trojnásobku hodnoty štandardnej odchýlky. Pri sezónnych anomáliách
...
jednotlivých meraní vzniká napr. metóda kĺzavého priemeru s~exponenciálnym
...
(pravdepodobnostné exponenciálne váhovanie), kde v~kombinácií s~metódou ESD
...
\noindent
Ďalšou metódou založenou na ESD je aj rozšírená S-H-ESD (angl. \textit{Enhanced
...
(angl. \textit{Locally Estimated Scatterplot Smoothing}), keďže klasická
...
funkcie $g(x)$, ktorá predikuje danú veličinu s~náhodnou chybou $\epsilon$.
...
vzdialenosti sme bližšie analyzovali už v~kapitole~\ref{c:distance-metrics}.
...
skóre, na základe ktorého bude daný časový rad považovaný za anomálny vo~vybranom
...
a~anomálnymi intervalmi v~pôvodnom časovom rade príznakov sú ostré, pomocou
...
radu sú výsledkom grafy zobrazené na obrázku~\ref{fig:smoothing-anomalies}.
...
Experimentmi sme však overili, že rozdiely sú štatisticky nevýznamné.
...
Rozdelenie je navrhnuté na základe porovnania charakteristík polohy datasetu
...
medzikvartilové pravidlo priemerne za 0.0197 sekundy. Za anomálnych je však
...
identifikovaných anomálnych odberateľov sa znížil približne na 2.66\%, pri
...
spracované a~upravené do formátu potrebného pre model. Po vytvorení prototypu
...
\newpage
\subsection{Plán do letného semestra}
\begin{table}[!h]
  \centering
  \begin{tabular}{||l|p{0.65\textwidth}||}
    \hline \hline
...
\dirtree{%
  ...
  .3 agg-functions.R.
  .3 dependencies.R.
  .3 filter-functions.R.
  .3 load-functions.R.
  .3 score-and-visualize-ad.R.
  .3 score-and-visualize-own.R.
  .3 utilities.R.
  .3 utilities.sh.
  .3 visualize-functions.R.
}
...
\begin{description}
  ...
  \begin{description}
    \item[-- graphs] adresár obsahujúci skripty vykresľujúce grafy, ktoré sú použité v~práci
    \item[-- server] adresár obsahujúci skripty, ktoré boli spúšťané na výpočtovom servery
...
Názov odovzdaného archívu: DP\_prilohy\_digital\_matus\_cuper.zip
