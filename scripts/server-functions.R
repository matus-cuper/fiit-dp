...
movingWindowWorkdaysRemoveBuildinZ <- function(windowSize = 4, clusterCount = 20, distanceMetric = "dtw_basic") {
  result <- data.frame()
  bar <- txtProgressBar(min = 0, max = WEEKS - windowSize, style = 3)
  counter <- 0
...
movingWindowHolidays <- function(windowSize = 4, clusterCount = 20, distanceMetric = "dtw_basic") {
  result <- list()
  bar <- txtProgressBar(min = 0, max = WEEKS - windowSize, style = 3)
  counter <- 0
  ...
  for (week in seq(3, WEEKS - windowSize)) {
    ...
    result[[week]] <- as.myS3class(tsclust(
      t(data.frame(lapply(aggregate(. ~ time, tmp[c("time", DATACOLUMNS)], FUN = mean)[DATACOLUMNS], norm_z))),
      k = clusterCount,
      distance = distanceMetric,
      args = tsclust_args(dist = list(window.size = clusterCount))
    ))
...
movingWindowWorkdays <- function(windowSize = 4, clusterCount = 20, distanceMetric = "dtw_basic") {
  ...
  for (week in seq(1, WEEKS - windowSize)) {
    s <- (week - 1) * (MPD * WEEK) + 1
    e <- WEEK * MPD * (week + windowSize - 1)
    tmp <- filterWorkdays(filterWeekdays(IRELAND[s:e, COLUMNS]))
...
movingWindowWorkdaysRemoveTime <- function(windowSize = 4, clusterCount = 20, distanceMetric = "dtw_basic") {
  result <- data.frame()
  bar <- txtProgressBar(min = 0, max = WEEKS - windowSize, style = 3)
  counter <- 0
  ...
  for (week in seq(1, WEEKS - windowSize)) {
    ...
    result <- rbind(
      result,
      cvi(tsclust(
        t(data.frame(lapply(aggregate(. ~ time, tmp[c("time", DATACOLUMNS)], FUN = mean), norm_z))[DATACOLUMNS]),
        k = clusterCount,
        distance = distanceMetric,
        args = tsclust_args(dist = list(window.size = clusterCount))
      ))
...
movingWindowWorkdaysRemoveZ <- function(windowSize = 4, clusterCount = 20, distanceMetric = "dtw_basic") {
  result <- data.frame()
  bar <- txtProgressBar(min = 0, max = WEEKS - windowSize, style = 3)
  counter <- 0
  ...
  for (week in seq(1, WEEKS - windowSize)) {
    ...
    result <- rbind(
      result,
      cvi(tsclust(
        t(data.frame(lapply(aggregate(. ~ time, tmp[c("time", DATACOLUMNS)], FUN = mean), as.numeric))),
        k = clusterCount,
        distance = distanceMetric,
        args = tsclust_args(dist = list(window.size = clusterCount))
      ))
...
setMethod(
  "as.myS3class",
  signature(object = "PartitionalTSClusters"),
  function(object)
  {
    structure(list(
      centroids = object@centroids,
      cldist = object@cldist,
      clusinfo = object@clusinfo,
